...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (username.length < 3) {
      setError("USERNAME TOO SHORT (MIN 3)")
      setLoading(false)
      return
    }
    ...
    // Attempt signup
    const success = signup(username.trim(), password, confirmPassword)
    ...
    if (success) {
      ...
      const existingUser = localStorage.getItem(`user_${username.trim()}`)
      if (existingUser) {
      ...
      <div className="flex flex-col gap-8 items-center z-10 max-w-md w-full px-4">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6 w-full">
          ...
          >
            ...
            <Link href="/code/lg-01" className="hover:opacity-100 transition-opacity underline">
              LOGIN
            </Link>
          </div>
        </form>
